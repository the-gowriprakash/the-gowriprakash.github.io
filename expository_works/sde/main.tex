...
\end{equation}
for almost every $\omega \in \Omega$. Here $\lambda$ is the Lebesgue measure on $[0,t]$. The integrand $\mu(s, X_s(\omega))$ is a function of $s$ for each fixed $\omega$ (i.e., a sample path). Therefore, by evaluating the integral for each sample path, we obtain a random variable $I_t: \Omega \to \mathbb{R}^n$.\\

The riemann integral counterpart being
\begin{equation}
    I_t=\int_0^t \mu(s, X_s)\,ds = \lim_{n \to \infty} \sum_{i=0}^{n-1} \mu(t_i, X_{t_i}) (t_{i+1} - t_i).
\end{equation}
Now in this case, $\mu(t_i, X_{t_i})$ is a random variable (i.e., $\mu \circ X_{t_i}$), and the sum $\sum_{i=0}^{n-1} \mu(t_i, X_{t_i}) (t_{i+1} - t_i)$ is a finite sum of random variables. In a sense, this provides a different picture of the computation process than the sample paths picture provided by the Lebesgue integral. Therefore one has options to make an algorithm to compute the integral, either by simulating sample paths or by using the riemann sum approximation i.e., setting a large finite $n$ instead of taking the limit.\\
...
\end{equation}
So, sort of like a riemann sum.
\begin{equation} 
    J_t = \int_0^t \sigma(s, X_s)\,dW_s := \lim_{n \to \infty} J_t^{(n)}.
\end{equation}    
I'm avoiding the description of the above convergence as it moves away from the scope of this exposition. For a detailed description, refer~\ref{app:C}.\\
...
What is the underlying distribution? Is there a `the' underlying distribution for a given dataset? Or is it something that depends on the model? i.e., how different models extrapolate information from the training set. The concept of an \textit{empirical distribution} explains what \textit{generation} is. Say our training dataset is $\{x_i\}_{i=1}^m \subset \mathbb{R}^n$. The corresponding empirical distribution $\mathbb{P}_e$ (which is a measure on the target space, which enables integration with respect to it) of this dataset would then be
\begin{equation}
...
The function $X_0$ is on which we employ the forward SDE $dX_t = \mu(t, X_t)\,dt + \sigma(t, X_t)\,dW_t$ to get a continuous time sequence $\{X_t\}_{t \geq 0}$\footnote{This notation is a doppelganger of the stochastic process notation, as the notation for a stochastic process $\{X_t\}_{t \geq 0}$ unfortunately lacks an indication of the probability distributions associated with the random variables.} where $X_t = X_0+I_t+J_t$ as demonstrated in the introduction \ref{eq:sde soln}.
But really, this does not have the full information of the solution stochastic process (refer~\ref{app:B}). 
The random varible $X_0$ that we are dealing with here, is really not a random variable in the measure-theoretic probability theory sense (as given in \ref{app:A}), as our sample space $\Omega$ is not a probability space by itself, for us to be able to push the measure forward (i.e., the pushforward measure) to the measurable space formed by $X_0$. 
In fact that's the whole problem of generative modeling, finding $p_0$. But the random varible not being such doesn't stop us from employing the SDE on it. 
...
This is where the Fokker--Planck equation (refer~\ref{app:D}) gets in the picture, the solution of which gives us the marginal probability distribution $p_t$ of $X_t$ for all $t \geq 0$ (i.e., a continuous time sequence $\{p_t\}_{t \geq 0}$), ofcourse, if we know $p_0$ to start with, as the deterministic evolution of marginal probabilities needs an initial condition to have the determinism.\\

The FPE correspondence of the foward SDE (let's call it forward FPE) $dX_t = \mu(t, X_t)\,dt + \sigma(t, X_t)\,dW_t$ (which is an $n$-dimensional euclidean SDE) is 
\begin{equation}
...
The reverse FPE in the canonical form (refer~\ref{app:D}) reads
\begin{equation}
...
So do these SDEs make the marginal converge to a known distribution when $t \to \infty$? For the VE SDE, No! As the variance grows unbounded as $t \to \infty$, the marginals of the VE SDE diverge, failing to converge to a proper probability distribution. But for the VP SDE, the marginals does converge, to the standard gaussian $\mathcal{N}(0,I)$.\\
...
A probability space is a triple $(\Omega, \mathcal{F}, P)$ where $\Omega$ is the sample space, $\mathcal{F}$ is a $\sigma$-algebra of subsets of $\Omega$, and $P: \mathcal{F} \to [0,1]$ is a probability measure.
\end{definition}
...
A random variable is a measurable function $X: \Omega \to Y$ (where $(Y, \mathcal{G})$ is a measurable space) defined on a probability space $(\Omega, \mathcal{F}, P)$.
\end{definition}
...
\begin{equation}
    \frac{\partial \tilde{p}_t}{\partial t} = \frac{\partial}{\partial t} p_{T-t}(x) = -\frac{\partial p_\tau}{\partial \tau}
\end{equation}

Because $p_\tau$ represents the forward density evaluated at time $\tau$, its derivative $\frac{\partial p_\tau}{\partial \tau}$ is governed exactly by the forward FPE evaluated at $\tau$. Substituting this yields:
